...
class raw_graph:
	def __init__(self, funcname, g, func_f):
		...
		self.attributing()  # map raw old cfg 2 g
	...
	def attributing(self):
		...
		for node in self.old_g:
			fvector = self.retrieveVec(node, self.old_g)  # 将old_g的特征map到一个list返回
	...
	# 为g添加每个节点的后继节点g.nodes[node]['offs']
	...
	def retrieveVec(self, id_, g):
		...
		################# modified #####################
		# # label 8
		# tmp = g.nodes[id_]['label']
		# feature_vec.append(tmp)
		# # numNc : num of consts & strings 9
		# tmp = g.nodes[id_]['numNc']
		# feature_vec.append(tmp)
		# # externs 10
		# tmp = g.nodes[id_]['enterns'] # externs
		# feature_vec.append(tmp)
